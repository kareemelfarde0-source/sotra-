  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (matched) {
        ...
        // Create demo tracked order if they type an arbitrary code like SOTRA-123456 or phone
        setFoundOrder({
          orderId: searchInput.toUpperCase().startsWith('SOTRA') ? searchInput.toUpperCase() : `SOTRA-${Math.floor(100000 + Math.random() * 900000)}`,
          date: 'اليوم / Today',
          items: [],
          customer: {
            name: isArabic ? 'عميل سوترة' : 'SOTRA Customer',
            phone: searchInput,
            whatsapp: searchInput,
            governorate: isArabic ? 'القاهرة / الجيزة' : 'Cairo / Giza',
            address: isArabic ? 'جاري تجهيز الشحنة للتسليم' : 'In fulfillment pipeline'
          },
          paymentMethod: 'cod',
          shippingFee: 45,
          subtotal: 980,
          ...
          total: 1025,
          status: 'shipping',
          statusAr: 'في طريق الشحن والتوصيل'
        });
      }
      setSearched(true);
      setLoading(false);
