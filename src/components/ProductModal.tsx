            ...
            <div>
              ...
              <div className="mb-5">
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider mb-2">
                  ...
                  {/* Dynamic stock indicator for chosen color & size */}
                  {(() => {
                    ...
                    if (currentStock <= 0) {
                      ...
                      return <span className="text-amber-600 font-bold text-[11px]">{isArabic ? `متبقي ${currentStock} قطع فقط!` : `Only ${currentStock} left!`}</span>;
                    }
                    return <span className="text-green-700 font-bold text-[11px]">{isArabic ? `متوفر (${currentStock} قطعة)` : `In Stock (${currentStock})`}</span>;
                  })()}
                </div>
                  ...
                  {product.sizes.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        title={`${s.size} - ${stockCount} ${isArabic ? 'قطع متوفرة' : 'pcs'}`}
